...
const handleDownloadPDF = async (purchaseId) => {
    try {
      const blob = await downloadTicketPDF(purchaseId);
      //  The rest of your code to trigger the download (same as before)
  ...
  const handleCancelPurchase = async (purchaseId) => {
    try {
      ...
      alert(
        `Purchase ${purchaseId} cancelled successfully. Refund Amount: ₹${res.refund_amount}`
      );
    } catch (error) {
      console.error("Failed to cancel purchase", error);
      setCancellationStatus((prev) => ({ ...prev, [purchaseId]: "error" })); // Set status
      alert(
        `Failed to cancel purchase ${purchaseId}.  ${
          ...
          <Card key={purchase.purchase_id} className="w-full">
            ...
            <CardContent>
              <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          <div>
                            <p>
                              ...
                              {getSeatingInfo(ticket).row}
                            </p>
                            <p>
                              <span className="font-semibold">Seat:</span>{" "}
                              {getSeatingInfo(ticket).seat}
                            </p>
                            <p>
                          ...
                          <div>
                            ...
                            <p>
                              ...
                              {dateString(purchase.show.date_time)}
                            </p>
                            <p>
                              <span className="font-semibold">Show Time:</span>{" "}
                              {timeString(purchase.show.date_time)}
                            </p>
                          </div>
                          <div>
                            <p>
                ...
                <Button
                  variant="secondary"
                  onClick={() => handleDownloadPDF(purchase.purchase_id)}
                >
                  Download Ticket PDF
                </Button>
              </div>
